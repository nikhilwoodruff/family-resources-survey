...
def consolidate(
    table: pd.DataFrame, max_count: int = 1, count_label: str = None, count_all = False
) -> pd.DataFrame:
    ...
    Args:
        ...
        count_all (bool): Whether to set max_count to the maximum number of occurrences for an entity.
    ...
    for i in range(max_count - 1 + (max_count == max_number)):
